...
body.rpp-cover-direct #cover #tocBtn{display:none!important}
...
<script>
(()=>{
  const HIDE_TEXT=['そして、11.15、11.18へ','OUR VOW, OUR JOURNEY','STORIES ↓','続きから読む','WRITE YOUR STORY｜私の記録を綴る','WRITE YOUR STORY｜原稿を書く','ログアウト'];
  ...
  const applyPublic=()=>{
    ...
    cover.querySelectorAll('*').forEach(el=>{const text=(el.textContent||'').trim();if(HIDE_TEXT.includes(text)||text.startsWith('前回：'))el.style.display='none'});
    const eyes=[...cover.querySelectorAll('.eyebrow')];eyes.forEach((el,i)=>{if(i>0&&(el.textContent||'').includes('OUR VOW'))el.style.display='none'});
