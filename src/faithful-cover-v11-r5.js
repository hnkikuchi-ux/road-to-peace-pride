...
(()=>{
  ...
  function polishEditor(){const editor=document.getElementById('editor'),form=document.getElementById('formArea');if(!editor||!form||form.dataset.r5Sections==='1')return;form.dataset.r5Sections='1';const field=(id)=>document.getElementById(id)?.closest('.field');const confirm=document.getElementById('confirm')?.closest('label');const stack=document.getElementById('submit')?.closest('.stack');makeSection('01','基本情報','記載日・組織名・氏名を入力してください。',[field('record_date'),field('org'),field('name')]);makeSection('02','あなたの記録','題名と、9.12までの挑戦・誓い・思いをそのまま綴ってください。',[field('title'),field('body')]);makeSection('03','写真・確認・提出','写真は任意です。内容を確認して提出してください。',[field('photo'),confirm,stack]);const submit=document.getElementById('submit');if(submit)submit.textContent='この内容で提出する'}
  function compactCodeCard(card){if(!card||card.dataset.r5Compact==='1')return;card.dataset.r5Compact='1';card.classList.add('rpp-code-collapsed');const h=card.querySelector('h2');if(h)h.textContent='編集用承認コード';const badge=document.createElement('div');badge.className='rpp-code-saved-badge';badge.textContent='保存後もここから確認できます';if(h)h.insertAdjacentElement('afterend',badge);const copy=card.querySelector('#rppCopyEditCode');const tools=document.createElement('div');tools.className='rpp-code-tools';const reveal=document.createElement('button');reveal.type='button';reveal.className='btn secondary';reveal.textContent='コードを表示';reveal.onclick=()=>{const on=card.classList.toggle('rpp-code-visible');reveal.textContent=on?'コードを隠す':'コードを表示'};tools.appendChild(reveal);if(copy){copy.textContent='コードをコピー';tools.appendChild(copy)};card.appendChild(tools)}
  function showCheckpoint(rawCode,reset){
    ...
    overlay.innerHTML='<div class="rpp-checkpoint-card"><div class="rpp-checkpoint-kicker">YOUR EDIT KEY</div><h2 id="rppCheckpointTitle">'+(reset?'新しい編集用承認コードを保存してください':'編集用承認コードを保存してください')+'</h2><p class="rpp-checkpoint-lead">このコードが、あなたの原稿をあとから編集するための「鍵」になります。原稿を書き始める前に保存してください。</p><div class="rpp-checkpoint-code">'+shown+'</div><button type="button" id="rppCheckpointCopy" class="btn secondary rpp-copy">承認コードをコピー</button><div id="rppCheckpointCopyState" class="rpp-copy-state"></div><label class="rpp-saved-check"><input id="rppCodeSaved" type="checkbox"><span><b>スクリーンショットまたはメモで保存しました</b><br>次回編集するときに、この6桁コードを使用します。</span></label><button type="button" id="rppCheckpointContinue" class="btn primary rpp-continue" disabled>保存しました。原稿を書く</button><p class="rpp-checkpoint-foot">紛失した場合は、登録したメールアドレスへの本人確認で新しいコードを再発行できます。</p></div>';
