...
async function sendOtpEmail(env,s,email,code){
  ...
  const body={sender:{email:cfg.senderEmail,name:cfg.senderName},to:[{email}],subject:'【ROAD TO PEACE PRIDE】認識コード',htmlContent:`<div style="font-family:sans-serif;line-height:1.8"><p>原稿投稿画面の認識コードです。</p><p style="font-size:30px;letter-spacing:.18em;font-weight:700">${code}</p><p>このコードは10分間有効です。</p><p style="color:#777;font-size:12px">ROAD TO PEACE PRIDE / MEMORIAL COLLECTION 2026</p></div>`};
...
async function api(request,env,path){
  ...
  if(path==='/api/auth/verify'&&request.method==='POST'){
    if(cfg.deadlinePassed)return json({error:'現在は原稿受付期間を終了しています。'},403);const b=await request.json().catch(()=>({})),email=cleanEmail(b.email),code=String(b.code||'').trim(),row=await env.DB.prepare('SELECT * FROM rpp_otps WHERE email=?').bind(email).first();if(!row||Date.parse(row.expires_at)<Date.now())return json({error:'認識コードの有効期限が切れています。'},401);if(Number(row.attempts||0)>=6)return json({error:'認証試行回数を超えました。コードを再送してください。'},429);const pepper=env.OTP_PEPPER||env.SETUP_KEY||(preview?'preview-pepper':''),got=await sha256(`${email}:${code}:${pepper}`);if(got!==row.code_hash){await env.DB.prepare('UPDATE rpp_otps SET attempts=attempts+1 WHERE email=?').bind(email).run();return json({error:'認識コードを確認してください。'},401)}await env.DB.prepare('DELETE FROM rpp_otps WHERE email=?').bind(email).run();const token=await createSession(env,'author',email,60*60*24*30);return json({ok:true,email},200,{'Set-Cookie':sessionCookie('rpp_author',token,60*60*24*30)});
  }
