...
async function sha256(value){
  const b=await crypto.subtle.digest('SHA-256',enc.encode(String(value)));
...
function previewMode(env){return String(env.PREVIEW_MODE||'true').toLowerCase()!=='false'}
...
async function deleteSession(env,request,cookieName){
  const token=cookieMap(request)[cookieName];if(token)await env.DB.prepare('DELETE FROM rpp_sessions WHERE token_hash=?').bind(await sha256(token)).run();
}
...
async function sendOtpEmail(env,email,code){
  ...
  const senderEmail=env.OTP_SENDER_EMAIL||'';
  if(!senderEmail)throw new Error('OTP_SENDER_EMAIL is not configured');
...
async function api(request,env,url,path){
  ...
  const preview=previewMode(env);

  if(path==='/api/health'&&request.method==='GET'){
    const row=await env.DB.prepare("SELECT COUNT(*) n FROM rpp_stories").first();
    return json({ok:true,preview,emailConfigured:Boolean(env.BREVO_API_KEY&&env.OTP_SENDER_EMAIL),viewerPasswordConfigured:Boolean(env.VIEWER_PASSWORD),stories:Number(row?.n||0)});
  }

  if(path==='/api/viewer/login'&&request.method==='POST'){
    const body=await request.json().catch(()=>({}));
    const expected=env.VIEWER_PASSWORD||(preview?'demo':'');
    ...
    const token=await createSession(env,'viewer',null,60*60*24*7);
    return json({ok:true,preview},200,{'Set-Cookie':sessionCookie('rpp_viewer',token,60*60*24*7)});
  }
  if(path==='/api/viewer/logout'&&request.method==='POST'){
    await deleteSession(env,request,'rpp_viewer');return json({ok:true},200,{'Set-Cookie':clearCookie('rpp_viewer')});
  }
  ...
  if(photoMatch&&request.method==='GET'){
    ...
    const obj=await env.MEDIA.getWithMetadata(row.photo_key,'arrayBuffer');if(!obj.value)return new Response('Not found',{status:404});
  ...
  if(path==='/api/auth/request'&&request.method==='POST'){
    ...
    const code=randomCode(), pepper=env.OTP_PEPPER||env.SETUP_KEY||'preview-pepper';
    const codeHash=await sha256(`${email}:${code}:${pepper}`),expires=new Date(Date.now()+10*60*1000).toISOString(),sent=nowIso();
    ...
    let mailed=false;
    try{mailed=await sendOtpEmail(env,email,code)}catch(e){if(!preview)return json({error:'認証メールを送信できませんでした。'},502)}
    if(!mailed&&!preview)return json({error:'メール送信サービスが未設定です。'},503);
    return json({ok:true,preview,previewCode:preview&&!mailed?code:undefined});
  }
  if(path==='/api/auth/verify'&&request.method==='POST'){
    const body=await request.json().catch(()=>({}));const email=cleanEmail(body.email),code=String(body.code||'').trim();
    const row=await env.DB.prepare('SELECT * FROM rpp_otps WHERE email=?').bind(email).first();
    if(!row||Date.parse(row.expires_at)<Date.now())return json({error:'認証コードの有効期限が切れています。'},401);
    if(Number(row.attempts||0)>=6)return json({error:'認証試行回数を超えました。コードを再送してください。'},429);
    const pepper=env.OTP_PEPPER||env.SETUP_KEY||'preview-pepper',got=await sha256(`${email}:${code}:${pepper}`);
    if(got!==row.code_hash){await env.DB.prepare('UPDATE rpp_otps SET attempts=attempts+1 WHERE email=?').bind(email).run();return json({error:'認証コードを確認してください。'},401)}
    await env.DB.prepare('DELETE FROM rpp_otps WHERE email=?').bind(email).run();
    const token=await createSession(env,'author',email,60*60*24*30);
    return json({ok:true,email},200,{'Set-Cookie':sessionCookie('rpp_author',token,60*60*24*30)});
  }
  if(path==='/api/auth/logout'&&request.method==='POST'){
    await deleteSession(env,request,'rpp_author');return json({ok:true},200,{'Set-Cookie':clearCookie('rpp_author')});
  }

  if(path==='/api/me/story'){
    const session=await getSession(env,request,'author','rpp_author');if(!session)return json({error:'投稿者認証が必要です。'},401);
    const email=session.subject;
    if(request.method==='GET'){
      const row=await env.DB.prepare('SELECT * FROM rpp_stories WHERE author_email=?').bind(email).first();return json({story:row||null,email,preview});
    }
    if(request.method==='PUT'){
      const b=await request.json().catch(()=>({}));
      const title=safeText(b.title,100),body=safeText(b.body,10000),name=safeText(b.name,200),status=b.status==='submitted'?'submitted':'draft';
      if([...title].length>40)return json({error:'題名は40字以内です。'},400);
      if(status==='submitted'&&(!name||!title||!body))return json({error:'氏名・題名・本文を入力してください。'},400);
      const existing=await env.DB.prepare('SELECT id,created_at,photo_key FROM rpp_stories WHERE author_email=?').bind(email).first();
      const id=existing?.id||crypto.randomUUID(),created=existing?.created_at||nowIso(),updated=nowIso();
      await env.DB.prepare(`INSERT INTO rpp_stories(id,author_email,record_date,soku,bunku,honbu,shibu,category,name,title,body,photo_key,status,created_at,updated_at)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(author_email) DO UPDATE SET record_date=excluded.record_date,soku=excluded.soku,bunku=excluded.bunku,honbu=excluded.honbu,shibu=excluded.shibu,category=excluded.category,name=excluded.name,title=excluded.title,body=excluded.body,status=excluded.status,updated_at=excluded.updated_at`).bind(id,email,safeText(b.record_date,20),safeText(b.soku,100),safeText(b.bunku,100),safeText(b.honbu,100),safeText(b.shibu,100),safeText(b.category,30),name,title,body,existing?.photo_key||null,status,created,updated).run();
      const story=await env.DB.prepare('SELECT * FROM rpp_stories WHERE author_email=?').bind(email).first();return json({ok:true,story});
    }
  }
  if(path==='/api/me/photo'&&request.method==='POST'){
    ...
    const email=session.subject;let row=await env.DB.prepare('SELECT id,created_at FROM rpp_stories WHERE author_email=?').bind(email).first();
  ...
  if(path==='/api/admin/login'&&request.method==='POST'){
    const b=await request.json().catch(()=>({}));const expected=env.ADMIN_PASSWORD||env.SETUP_KEY||(preview?'654321':'');
    if(!expected)return json({error:'管理者パスワードが未設定です。'},503);if(String(b.password||'')!==String(expected))return json({error:'管理者認証を確認してください。'},401);
  ...
  if(path==='/api/admin/stats'&&request.method==='GET'){
    if(!await getSession(env,request,'admin','rpp_admin'))return json({error:'管理者認証が必要です。'},401);
    const total=await env.DB.prepare('SELECT COUNT(*) n FROM rpp_stories').first();const submitted=await env.DB.prepare("SELECT COUNT(*) n FROM rpp_stories WHERE status='submitted'").first();const draft=await env.DB.prepare("SELECT COUNT(*) n FROM rpp_stories WHERE status='draft'").first();const photos=await env.DB.prepare('SELECT COUNT(*) n FROM rpp_stories WHERE photo_key IS NOT NULL').first();
    return json({total:Number(total?.n||0),submitted:Number(submitted?.n||0),draft:Number(draft?.n||0),photos:Number(photos?.n||0),preview,emailConfigured:Boolean(env.BREVO_API_KEY&&env.OTP_SENDER_EMAIL)});
  }
...
export default {
  async fetch(request,env){
    ...
    try{
      if(path.startsWith('/api/')){const res=await api(request,env,url,path);if(res)return res}
      return env.ASSETS.fetch(request);
    }catch(error){console.error(error);return json({error:previewMode(env)?String(error?.message||error):'処理に失敗しました。'},500)}
  }
};
