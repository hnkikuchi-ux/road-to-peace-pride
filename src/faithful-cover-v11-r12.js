...
(()=>{
 ...
 function apply(){
   ...
   if(photoField){const l=photoField.querySelector('label');if(l)l.textContent='写真';let n=photoField.querySelector('.r12-photo-note');if(!n){n=document.createElement('div');n.className='r12-photo-note';n.innerHTML='<b>思い出の写真を1枚添付できます。</b><br>9.12までの歩みや、心に残る一枚をぜひ添付してください。写真は任意です。';photoField.querySelector('#photo')?.insertAdjacentElement('afterend',n)}const old=document.getElementById('photoOptionalNote');if(old)old.style.display='none'}
   ...
   const submit=document.getElementById('submit');if(submit)submit.textContent=submitted?'変更内容を再提出する':'この内容で提出する';
   const prev=document.getElementById('previewBtn');if(prev)prev.textContent='掲載イメージを確認';
   const save=document.getElementById('save');if(save)save.textContent=submitted?'変更内容を保存':'下書き保存';
 ...
 ready(()=>{installFetch();const run=()=>{apply();previewEnhance()};run();requestAnimationFrame(run);addEventListener('pageshow',run)});
})();
</script>`;
