const STORAGE_KEY = 'gikaiAiHpCloudPilotV2';
const urlParams = new URLSearchParams(location.search);
const pathSlugMatch = location.pathname.match(/^\/p\/([^/]+)\/?$/);
const publicSlug = pathSlugMatch ? decodeURIComponent(pathSlugMatch[1]) : (urlParams.get('slug') || '');
const adminMode = urlParams.get('admin') === '1' || (!publicSlug && location.pathname === '/');
const MANAGE_KEY_STORAGE = 'giinHomeManageKeyV2';
const REMINDER_STORAGE = 'giinHomeReminderShownDateV2';
let manageKey = urlParams.get('key') || localStorage.getItem(MANAGE_KEY_STORAGE) || '';
let currentSlug = publicSlug || localStorage.getItem('giinHomeSlugV2') || '';
let accountCreatedAt = '';
let latestSharePayload = null;
let cloudSyncPromise = null;
let cloudSyncTimer = null;
if (urlParams.get('key')) {
  localStorage.setItem(MANAGE_KEY_STORAGE, manageKey);
...
function publicPageUrl(slug = currentSlug) {
  return slug ? `${location.origin}/p/${encodeURIComponent(slug)}` : location.origin;
}

async function apiRequest(path, options = {}, requireKey = true) {
  const headers = new Headers(options.headers || {});
  if (requireKey && manageKey) headers.set('X-Manage-Key', manageKey);
  const response = await fetch(path, { ...options, headers });
  const type = response.headers.get('content-type') || '';
  const payload = type.includes('application/json') ? await response.json() : await response.text();
  if (!response.ok) throw new Error(payload?.error || String(payload) || '通信に失敗しました。');
  return payload;
}
...
async function uploadImageValue(value) {
  ...
  if (blob.size > 5 * 1024 * 1024) throw new Error('写真が大きすぎます。別の写真を選んでください。');
  const result = await apiRequest('/api/manage/image', { method: 'POST', headers: { 'Content-Type': blob.type || 'image/jpeg' }, body: blob });
  return result.url;
}

async function prepareStateForCloud(source) {
  const next = clone(source);
  next.profile.image = await uploadImageValue(next.profile.image);
  for (const item of next.achievements || []) item.image = await uploadImageValue(item.image);
  for (const post of next.posts || []) {
    post.images = await Promise.all((post.images || []).map(uploadImageValue));
  }
  if (next.draft) next.draft.images = await Promise.all((next.draft.images || []).map(uploadImageValue));
  return next;
}

async function syncStateToCloud({ quiet = false } = {}) {
  if (!manageKey) return false;
  if (cloudSyncPromise) return cloudSyncPromise;
  cloudSyncPromise = (async () => {
    document.body.classList.add('syncing');
    const prepared = await prepareStateForCloud(state);
    const result = await apiRequest('/api/manage/state', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ state: prepared }) });
    state = normalizeState(prepared);
    currentSlug = result.slug || currentSlug;
    if (currentSlug) localStorage.setItem('giinHomeSlugV2', currentSlug);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
    updateCloudUi('クラウドに保存されています。');
    return true;
  })().catch(error => {
    updateCloudUi(`端末には保存済みです。クラウド保存エラー：${error.message}`);
    if (!quiet) showToast(error.message || 'クラウドに保存できませんでした');
    return false;
  }).finally(() => {
...
function scheduleCloudSync() {
  ...
  cloudSyncTimer = setTimeout(() => void syncStateToCloud({ quiet: true }), 350);
}

async function loadCloudState() {
  if (publicSlug) {
    const result = await apiRequest(`/api/public/${encodeURIComponent(publicSlug)}`, {}, false);
    currentSlug = result.slug || publicSlug;
    accountCreatedAt = result.createdAt || '';
    state = normalizeState(result.state || {});
    return true;
  }
  if (manageKey) {
    const result = await apiRequest('/api/manage/state');
...
function updateCloudUi(message) {
  ...
  const card = $('publicLinkCard');
  if (card && currentSlug) {
    card.classList.remove('hidden');
    $('publicLinkName').textContent = `${state.profile.name || '議員'}さんの公開ページ`;
    $('publicUrlText').textContent = publicPageUrl();
  }
}

function latestPublishedDate() {
  const published = (state.posts || []).filter(post => post.status !== 'draft');
  const times = published.map(post => new Date(post.createdAt || `${post.date || ''}T00:00:00`).getTime()).filter(Number.isFinite);
  const fallback = accountCreatedAt ? new Date(accountCreatedAt).getTime() : Date.now();
  return new Date(times.length ? Math.max(...times) : fallback);
}

function checkUpdateReminder() {
  if (!adminMode || publicSlug) return;
  const last = latestPublishedDate();
  const elapsed = Math.floor((Date.now() - last.getTime()) / 86400000);
  if (elapsed < 5) return;
  const today = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Tokyo' }).format(new Date());
  if (localStorage.getItem(REMINDER_STORAGE) === today) return;
  localStorage.setItem(REMINDER_STORAGE, today);
  $('updateReminderText').textContent = `前回の更新から${elapsed}日たっています。短い報告でも大丈夫です。`;
...
async function buildSharePayload(post) {
  const text = `${post.title}\n\n${post.body}\n\n${publicPageUrl()}`;
  const payload = { title: post.title, text, url: publicPageUrl() };
  const files = [];
  for (let i = 0; i < (post.images || []).length; i++) {
    const source = post.images[i];
    try {
      const blob = await fetch(source).then(r => r.blob());
      files.push(new File([blob], `activity-${i + 1}.jpg`, { type: blob.type || 'image/jpeg' }));
    } catch {}
  }
...
async function publishCurrentActivity(shareAfter) {
  ...
  if (manageKey && !saved) return;
  latestSharePayload = await buildSharePayload(post);
  renderSite();
  resetActivityForm();
  setView('site');
  ...
  if (shareAfter) {
    const shared = await sharePayload(latestSharePayload);
    if (!shared) $('shareFallbackModal').classList.remove('hidden');
  }
}
...
function setupEvents() {
  ...
  $('previewSiteButton').addEventListener('click', () => currentSlug ? window.open(publicPageUrl(), '_blank', 'noopener') : setView('site'));
  $('dashboardPreviewButton').addEventListener('click', () => currentSlug ? window.open(publicPageUrl(), '_blank', 'noopener') : setView('site'));
  $('copyPublicUrlButton').addEventListener('click', () => void copyText(publicPageUrl(), '公開URLをコピーしました'));
  $('openPublicUrlButton').addEventListener('click', () => window.open(publicPageUrl(), '_blank', 'noopener'));
  $('writeNowButton').addEventListener('click', () => { $('updateReminderModal').classList.add('hidden'); setView('admin'); setAdminTab('activity'); });
  $('closeReminderButton').addEventListener('click', () => $('updateReminderModal').classList.add('hidden'));
  $('openShareSheetButton').addEventListener('click', async () => { if (await sharePayload(latestSharePayload)) $('shareFallbackModal').classList.add('hidden'); });
  $('copyShareTextButton').addEventListener('click', () => void copyText(latestSharePayload?.text || '', 'SNS投稿用の文章とURLをコピーしました'));
  $('closeShareModalButton').addEventListener('click', () => $('shareFallbackModal').classList.add('hidden'));
  ...
  $('publishButton').addEventListener('click', () => void publishCurrentActivity(false));
  $('publishShareButton').addEventListener('click', () => void publishCurrentActivity(true));
...
async function init() {
  try {
    ...
    updateCloudUi(manageKey ? 'クラウドに保存されています。' : (publicSlug ? '' : 'この端末だけの試用モードです。'));
  } catch (error) {
    updateCloudUi(`クラウドから読み込めませんでした：${error.message}`);
    if (publicSlug) {
      document.body.innerHTML = `<main class="load-error"><h1>公開ページを表示できません</h1><p>${escapeHtml(error.message)}</p></main>`;
      return;
    }
  }
  renderSite();
  ...
  if (publicSlug) document.querySelector('.app-header')?.classList.add('hidden');
  setTimeout(checkUpdateReminder, 700);
